 ...
 */
int		c_line;			/* Current line number		*/
...
int Preprocess(const char*in,const char*out)
{
  ...
  /* cflag = TRUE;
  keepcomments = TRUE; */
  eflag = TRUE;		/* Ignore non-fatal errors.		*/
 ...
 */
{
	...
	for (;;) {
		...
		switch (type[c]) {
		case LET:
		    fputs(token, fout);	/* Quite ordinary token	*/
		    break;
 ...
 */
{
	...
	if (keepcomments)			/* Make sure # comes on	*/
	    fputc('\n',fout);			/* a fresh, new line.	*/
	fprintf(fout,"#%s \"%s\" %d", LINE_PREFIX, infile->filename, c_line);
	/*if (infile->fp != NULL) {
