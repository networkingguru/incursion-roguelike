...
ProcessSpells:
    ...
    do {
        ...
        Write(WinLeft() + (WinSizeX() / 2) - (strlen(WinTitle)/2),0,WinTitle);
...
int sortItemsGrouped(const void *a, const void *b) {
    ...
    int16 oA = sortOrder(A), oB = sortOrder(B);
    if (oA != oB)
        return oA - oB;
...
void TextTerm::InvShowSlots(bool changed) {
    ...
    for (i = 0; i != NUM_SLOTS && changed; i++) {
        ...
        if (i)
            str = Format("%c)%c%s%c :%s", SlotLetters[i - 1], TMON(p->mID)->HasSlot(i) ? -GREY : -BLUE, SlotNames[i], -GREY, (const char*)str);
        else
    ...
    if (currCon && changed) {
        ...
        if (currCon->HasStati(LOCKED))
            ...
            for (j = 0, y = 1; Contents[j]; j++, y++) {
                ...
                str = Format("  %02d) %s\n", j + 1, (const char*)Contents[j]->Name(NA_LONG | NA_MECH));
...
#define INV(sl) (sl < NUM_SLOTS ? p->Inv[sl] : Contents[sl-NUM_SLOTS]->myHandle)
    ...
    if (theChest)
        if (theChest->x != p->x || theChest->y != p->y || theChest->m != p->m)
            theChest = NULL;
...
ResetCurrCon:
    if (theChest) {
        ...
        if ((TMON(p->mID)->HasSlot(SL_PACK) || p->AbilityLevel(CA_WILD_SHAPE)) && p->Inv[SL_PACK])
            if (oItem(p->Inv[SL_PACK])->Type == T_CONTAIN)
                currCon = (Container*)(oItem(p->Inv[SL_PACK]));

        if (currCon == NULL) {
            for (cs = 0; cs != SL_LAST; cs++)
                if (p->Inv[cs] && oItem(p->Inv[cs])->isType(T_CONTAIN) && (p->InSlot(cs) || p->AbilityLevel(CA_WILD_SHAPE))) {
...
Redraw:
    ...
    GotoXY(0, WinSizeY() - 1);
    Color(EMERALD);
    Write(Format("[Num] Buy Item [B]uy Selected [S]ell Item [ESC] Quit %s", Seller->HasMFlag(M_SELLER) ? "Shopping" : "Bartering"));
...
PartialRedraw:
    ...
    for (i = 0; i != c; i++) {
        ...
        if (i <= lItem) {
            Color(GREY);
            SWrite(Format("%s%d) %c%s%c", i >= 10 ? "" : "0", i, (i == curr) ? -YELLOW : -GREY, (const char*)WareNames[i], -GREY));
            SGotoXY(width - 10, i);
            Color(GREY);
            SWrite(Format("%d gp", oItem(Wares[i])->getShopCost(p, Seller) / oItem(Wares[i])->GetQuantity()));
        } else {
...
BuySpell:
    ...
    if (TEFF(spID)->ManaCost > Seller->cMana()) {
        p->IPrint("The <Obj> doesn't have the mana to cast that right now -- come back later!", Seller);
        goto Redraw;
    }
    p->IPrint("The <Obj> casts <9><Res><7> on your behalf.", Seller, spID);
    p->LoseMoneyTo(cost, Seller);
...
SellItem:
    ...
    hObj hItem;
    hItem = LMenu(MENU_2COLS | MENU_ESC | MENU_BORDER, "Sell Which Item?", WIN_MENUBOX);
    if (hItem == -1) {
        SetWin(WIN_INVEN);
        Clear();
        SetMode(MO_PLAY);
...
#define SKILLS_SHOWN 15
    ...
    int8 Alloc[SK_LAST][6], Ranks[SK_LAST], Show[SK_LAST * 2], MRanks[SK_LAST], Spent[6], offset, last_class, last_universal, last_imp;
    int16 ch;
    bool maxed;
    String desc;
    TClass *tc;
...
Reset:
    ...
    Spent[0] = Spent[1] = Spent[2] = Spent[3] = Spent[4] = Spent[5] = 0;
    usp[0] = usp[1] = usp[2] = usp[3] = usp[4] = usp[5] = 0;
...
Recount:
    ...
    do {
        ...
        for (i = 1; i != SK_LAST; i++)
            if (tr->HasSkill(i) || p->HasStati(EXTRA_SKILL, i) || isFamiliar(i))
                if (!tc->HasSkill(i))
        ...
        for (i = 1; i != SK_LAST; i++)
            if (!(tc->HasSkill(i) || tr->HasSkill(i) || isFamiliar(i) || p->HasStati(EXTRA_SKILL, i)))
                if (SkillInfo[i].imp || 1)
                    Show[j++] = i;
        last_imp = j;
    ...
    ReDraw:
        ...
        for (i = offset, j = 0; Show[i] && j < SKILLS_SHOWN; i++)
            if (Show[i]) {
                ...
                static const char *AttrNames[] = { "Str", "Dex", "Con", "Int", "Wis", "Cha", "Luc" };
        ...
        do {
            ...
            case KY_ENTER:
                ...
                for (i = 0; i != SK_LAST; i++)
                    p->SkillRanks[i] += Alloc[i][0] + Alloc[i][1] + Alloc[i][2] + Alloc[i][3] + Alloc[i][4] + Alloc[i][5];
...
void TextTerm::OptionManager(int16 start_cat) {
    ...
    RedrawList:
        ...
        Color(RED);
        if (Sec == OPC_MACROS || Sec == OPC_INPUT || Sec == OPC_DISPLAY || Sec == OPC_STABILITY || Sec == OPC_LIMITS)
        ...
        default:
            if (Sec == OPC_MACROS && ch >= FN(1) && ch <= FN(12)) {
                ...
                for (i = 0; Candidates[i]; i++)
                    LOption(NAME(Candidates[i]), Candidates[i], DESC(Candidates[i]));
                mID = LMenu(MENU_2COLS | MENU_DESC | MENU_BORDER | MENU_ESC, "Bind which Macro?");
                if (mID == -1)
                    goto BadChar;
                k = ch - FN(1);
                if (ControlKeys & CONTROL)
    ...
    do {
        ...
        case 'A':
            if (p->NextLevXP() > (p->XP - p->XP_Drained) && !p->Opt(OPT_FREE_ADV)) {
                Message(Format("You have %d experience, and you need %d to reach level %d.", (p->XP - p->XP_Drained), p->NextLevXP(), p->TotalLevel() + 1));
                break;
            }
            p->AdvanceLevel();
            update = 1;
        ...
        case 'D':
            ...
            i = (int16)LMenu(MENU_ESC | MENU_BORDER | MENU_3COLS | MENU_DESC, "Desired Alignment", WIN_MENUBOX,
                "State the alignment your character aspires to live in accord with and Incursion will evaluate your actions in that context.");
            if (i != -1)
        ...
        case 'P':
            if (p->TotalLevel() < 3) {
                p->IPrint("You need at least three levels in core classes before you can select a prestige class.");
                break;
            }
            if ((p->ClassID[2] && TCLASS(p->ClassID[2])->HasFlag(CF_PRESTIGE)) || (p->ClassID[1] && TCLASS(p->ClassID[1])->HasFlag(CF_PRESTIGE))) {
        ...
        case 'M':
            for (q = 0; q != MAX_MODULES; q++)
                if (theGame->Modules[q]) {
                    ...
                    for (; xID != endID; xID++) {
                        if (xID == p->ClassID[0] || xID == p->ClassID[1] || xID == p->ClassID[2])
                            continue;
                        ...
                        if (TCLASS(xID)->HasFlag(CF_PSEUDO))
                            continue;
                        Candidates[i] = xID;
                        LOption((const char*)NAME(xID), i, DESC(xID) ? (const char*)DESC(xID) : "Description not found.");
                        i++;
                    }
                }
            if (!i)
                ...
                if (TCLASS(Candidates[i])->GetList(ALLOWED_GODS, GodList, 49)) {
                    for (int16 j = 0; GodList[j]; j++)
                        if (p->GodID == GodList[j])
                            goto GodIsOkay;
                    p->IPrint("Your worship of <Res> precludes you from that class.", p->GodID);
                    goto CannotTrain;
                GodIsOkay:;
                }
            }
            ...
            if (!TCLASS(Candidates[i])->HitDie) {
                ...
                break;
            } else if (TCLASS(Candidates[i])->PEvent(EV_ISTARGET, p, Candidates[i]) == ABORT) {
        ...
        case 'C':
        {
            ...
            StatiIter(p)
                if (S->Nature == POISONED || S->Nature == DISEASED) {
                    ...
                    s += Format(" You have made %d successful saving throws against this affliction to date.\n\n", S->Mag);
                }
            if (S->eID) {
                bool found = false;
                for (i = 0; i < num_men; i++)
            ...
            if (S->Source == SS_ENCH && S->eID && S->h) {
                s += Format("%cSpell Effect: %c%s (from %s, %s)\n%c  %s\n\n",
                    ...
                    (S->Duration > 0 ? (const char*)Format("duration %d", S->Duration) : (S->Duration == -2 ? "persistant" : "permanent")),
                    -GREY, (const char*)XPrint(DESC(S->eID)));
            } else if (S->Source == SS_ENCH && S->eID) {
                s += Format("%cSpell Effect: %c%s\n%c  %s\n\n", -SKYBLUE, -SKYBLUE, NAME(S->eID), -GREY, XPrint(DESC(S->eID)));
                ...
                s += Format("%cCurse: %c%s\n%c  %s\n\n", -SKYBLUE, -SKYBLUE, NAME(S->eID), -GREY, (const char*)XPrint(DESC(S->eID)));
            }
            StatiIterEnd(p)
