  ...
  switch (type) {
    ...
    case OrderWalkNearMe:
    {
      ...
      /* WHY GetQuiet AND NOT Get. A target keeps the handle of whatever the
         ...
         thing dies -- TargetSystem::Retarget simply builds a fresh list
         later. So a dead handle here is the ordinary state of affairs
         between retargets, not a fault, and this function's job is to answer
         "gone" rather than to report a defect.

         Registry::Get() logs "invalid object handle" for a handle whose
         object is gone. That is right for a caller who believes the handle
         is live, and wrong for this one. The fix above (inc-upw.13) needs
         the object itself in order to test its type, so it cannot go back to
         asking Registry::Exists() first -- Exists() and Get() walk the same
         hash chain, and Registry::Get is upstream's second-hottest function
         (inc/Defines.h, above OBJ_TABLE_SIZE), while this function runs
         inside a qsort comparator. GetQuiet() is the same single walk with
         the complaint left out.

         Measured, inc-upw.39: with Get() here, seeds 3, 25 and 26 of the
         40-seed gate logged eight "invalid object handle" lines between
         them. With GetQuiet() they log none, and no other gate number
         moves. */
      if (!data.Creature.c) return NULL;
      {
        Object *o = theRegistry->GetQuiet(data.Creature.c);
    ...
    case TargetItem:
      /* Same wrong-type risk as above, for the item handle in data.Item.i,
         and the same reason to look it up quietly. */
      if (!data.Item.i) return NULL;
      {
        Object *o = theRegistry->GetQuiet(data.Item.i);
