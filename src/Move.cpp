...
EvReturn Creature::Walk(EventInfo &e) {
	...
	switch (e.Event) {
	case EV_JUMP:
		if (HasStati(GRAPPLED) || HasStati(GRABBED) || HasStati(GRAPPLING) || HasStati(PRONE) || HasStati(STUCK)) {
...
SkipConfirms:
	...
	if ((m->At(x, y).Terrain != m->At(tx, ty).Terrain) || (isIllTer && (m->PTerrainAt(tx, ty, this) != m->PTerrainAt(x, y, this)))) {
...
#define BANK_AMOUNT     5
	...
	if (m->SolidAt(ox, oy) != m->SolidAt(tx, ty))
		if (HasAbility(CA_EARTHMELD) && onPlane() <= PHASE_VORTEX) {
			if (m->SolidAt(ox, oy))
				IDPrint("You step out of the rock.", "From out of solid stone, an <Obj>!", this);
			else
				IDPrint("You meld into the rock.", "The <Obj> melds into the rock.", this);
		}
	...
	* generic trap avoidance check under TriggerTrap */
	...
	if (isPlayer() && e.Event == EV_MOVE)
		if (HasAbility(CA_SHARP_SENSES) || HasAbility(CA_STONEWORK_SENSE) || HasStati(PERCEPTION))
			for (i = 0; i != 8; i++)
				if (d = m->FDoorAt(x + DirX[i], y + DirY[i])) {
					...
					if (!SkillCheck(SK_SEARCH, 15, true, d->GetStatiMag(RETRY_BONUS, SK_SEARCH, this), "retry")) {
	...
	if (tt->HasFlag(TF_FALL))
		if (!isAerial()) {
			if (isIllTer && !tt2->HasFlag(TF_FALL)) {
				...
				IDPrint("You fall...", "An <Obj> falls into the <Res>!", this, m->TerrainAt(x, y));
			RemoveStati(CHARGING);
			if (isPlayer()) {
