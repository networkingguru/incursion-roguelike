	...
	case SK_HEAL: {
		...
		case AD_DISE:
			StatiIterNature(this, ((choice == AD_POIS) ? POISONED : DISEASED))
				...
				else if (LastSkillCheckResult > (healDC - 5) + (te->Vals(0)->lval - S->Mag) * 5) {
	...
	MapIterate(m, t, i)
		if (dist(t->x, t->y, x, y) <= 5) {
			if (t->isType(T_DOOR)) {
				...
				if (!SkillCheck(SK_SEARCH, 15, true, t->GetStatiMag(RETRY_BONUS, SK_SEARCH, this), "retry")) {
					think = true;
				...
				if (isPlayer())
					if (!(m->At(t->x, t->y).Visibility & VI_VISIBLE))
						continue;
				if (!SkillCheck(SK_SEARCH, 20 - TEFF(((Trap*)t)->tID)->HasFlag(EF_MUNDANE) * 5, true, t->GetStatiMag(RETRY_BONUS, SK_SEARCH, this), "retry")) {
