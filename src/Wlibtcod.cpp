...
static int ls_octant = -1;      // octant currently held, -1 = released
                         ...
                         bool overview)
{
    ...
    static const Uint32 AUTORUN_DELAY_MS = 500;     // hold this long before repeat begins
    static const Uint32 AUTORUN_INTERVAL_MS = 120;  // repeat cadence after the delay (tune on device)
    ...
    *out = TCOD_key_t();
    ...
    // A fired right stick must cross the inner radius before another octant can fire.
    ...
    int dir = gamepad_dir8_from_axes(lx, ly, outer);
    if (left_magnitude < (long long)inner * inner) {
        ls_octant = -1;
        ls_repeating = false;
    }
    if (dir != -1) {
        if (dir != ls_octant) {
            ...
            ls_fire_ms = now;
            ls_repeating = false;
            out->vk = direction_keys[dir];
            return true;
        }
        if (allow_repeat) {
            Uint32 due = ls_repeating ? AUTORUN_INTERVAL_MS : AUTORUN_DELAY_MS;
            if (now - ls_fire_ms >= due) {
                ls_fire_ms = now;
                ls_repeating = true;
                out->vk = direction_keys[dir];
                return true;
...
/* Errors used to open a modal prompt that blocked until the player pressed
   B, E or C. Movement keys did nothing there, so a single error froze the
   game. The message now goes to logs/errors.log and play continues. Set
   INCURSION_ERROR_PROMPT to restore the old prompt. */
void Error(const char*fmt,...) {
	va_list argptr;
	char ch;
    int attempts = 0;
...
retry:
	...
	/* __buff2 is 80 bytes and __buffer is 1600. These were sprintf and
	   overflowed on any message longer than about 40 characters. */
#ifdef DEBUG
