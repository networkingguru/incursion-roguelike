...
// TODO: Remove HotGK specific code.
    ...
    if (!e.EParam) {
        ...
        for (i = 0, n = 0; Choices[i].Text; i++) {
            ...
            case EV_BARTER:
                ...
                break;
            case EV_COW:
                if (!e.EVictim->isHostileTo(this) || !HasSkill(SK_INTIMIDATE))
                    continue;
            ...
            case EV_DISMISS:
                ...
                break;
            case EV_DISTRACT:
                if (e.EVictim->isFriendlyTo(this) || e.EVictim->HasStati(DISTRACTED))
                    continue;
                break;
            case EV_ENLIST:
                if (e.EVictim->isHostileTo(this) || e.EVictim->isFriendlyTo(this))
                    continue;
                break;
            case EV_FAST_TALK:
                if (!e.EVictim->isHostileTo(this) || !e.EActor->HasSkill(SK_BLUFF))
                    continue;
                break;
            case EV_GREET:
                ...
                break;
            case EV_TERMS:
                if (!e.EVictim->isHostileTo(this) || !e.EVictim->HasStati(AFRAID))
                    continue;
                break;
            case EV_ORDER:
                if (!e.EVictim->isFriendlyTo(this))
            ...
            case EV_SURRENDER:
                ...
                break;
            case EV_TAUNT:
                if (e.EVictim->isFriendlyTo(this) || e.EVictim->HasStati(ENRAGED))
                    continue;
                break;
            }
            open[n++] = i;
...
EvReturn Creature::Barter(EventInfo &e) {
    ...
    if ((mod = getSocialMod(e.EVictim, false)) < 0 && !e.EVictim->HasMFlag(M_SELLER)) {
        ...
        e.EVictim->GainPermStati(TRIED, this, SS_MISC, EV_BARTER * 100 + SK_DIPLOMACY, 0);
    NoDesire:
...
EvReturn Creature::Cow(EventInfo &e) {
    ...
    if (e.EVictim->HasStati(TRIED, SK_INTIMIDATE + EV_COW * 100, this)) {
        e.EActor->IPrint("You've already tried to cow the <Obj>.", e.EVictim);
        return ABORT;
    }
    ...
    if (!e.EVictim->isMType(MA_EVIL))
        if (!(e.EActor->isMType(MA_ORC) || e.EActor->LevelAs(FIND("barbarian"))))
...
EvReturn Creature::OfferTerms(EventInfo &e) {
    ...
    if (e.EVictim->HasStati(TRIED, SK_DIPLOMACY + EV_TERMS * 100, this)) {
        e.EActor->IPrint("You've already offered terms to the <Obj>.", e.EVictim);
        return ABORT;
    }
...
EvReturn Creature::Dismiss(EventInfo &e) {
    ...
    e.EVictim->GainPermStati(TRIED, e.EActor, SS_MISC, SK_DIPLOMACY + EV_DISMISS * 100, result);
...
EvReturn Creature::Distract(EventInfo &e) {
    ...
    c = 0; BestRes = 0;
    MapIterate(m, cr, i)
        if (cr->isCreature() && (cr->isHostileTo(this) || cr->isFriendlyTo(e.EVictim))) {
            if (cr == e.EActor)
                continue;
            c++;
        ...
        MapIterate(m, cr, i)
            if (cr->isCreature() && cr->isHostileTo(this))
                if (cr == e.EVictim || doGroup)
                    cr->IncStatiMag(TRIED, SS_MISC, SK_BLUFF + EV_DISTRACT * 100, e.EActor, +5);
        return DONE;
    }

    MapIterate(m, cr, i)
        if (cr->isCreature() && (cr->isHostileTo(this) || cr->isFriendlyTo(e.EVictim)) && m->LineOfSight(x, y, cr->x, cr->y, NULL))
            if ((cr == e.EVictim || doGroup) && cr != e.EActor) {
                cr->IncStatiMag(TRIED, SS_MISC, SK_BLUFF + EV_DISTRACT * 100, e.EActor, +2);
                cr->GainTempStati(DISTRACTED, this, 2, SS_MISC, 0, LastSkillCheckResult - CheckDC);
                cr->IDPrint("You get distracted!", "The <Obj> looks away!", cr);
            }
...
EvReturn Creature::Greet(EventInfo &e) {
    ...
    if (e.EVictim->isHostileTo(e.EActor)) {
        IPrint("The <Obj> isn't interested in idle chatter; <he:Obj> wants to kill you!", e.EVictim, e.EVictim);
        return DONE;
    } else
...
EvReturn Creature::Surrender(EventInfo &e)
  {
    ...
    IPrint("The <Obj><Str> accepts your surrender and lets you go on your way.", e.EVictim, c > 1 ? "'s group" : "");    
    ...
    MapIterate(m,cr,i)
      if (cr->isCreature() && cr->isFriendlyTo(e.EVictim))
        if (doGroup || cr == e.EVictim) {
            ...
            cr->IDPrint("","The <Obj> flies into a mad rage at the <Obj>!", cr, e.EActor);
        }
...
bool Creature::isPMType(int32 mt, Creature *cr) {
    ...
    if (mt == MA_GOOD || mt == MA_EVIL || mt == MA_LAWFUL || mt == MA_CHAOTIC)
      if (SkillLevel(SK_BLUFF) > cr->SkillLevel(SK_APPRAISE))
        return cr->isMType(mt);
  
    if (HasStati(DISGUISED)) {
      rID spID;
      if (GetStatiMag(DISGUISED) < cr->SkillLevel(SK_SPOT) + cr->SkillLevel(SK_APPRAISE))
        goto IgnoreDisguise;
      if (cr->HasAbility(CA_SHARP_SENSES) && GetStatiVal(DISGUISED) < 8)
        goto IgnoreDisguise;
      if (cr->HasAbility(CA_SCENT) && GetStatiVal(DISGUISED) < 12)
        goto IgnoreDisguise;
...
int SortMoney(const void *a, const void *b) {
    ...
    return TITEM(B->iID)->Cost - TITEM(A->iID)->Cost;
}
...
int32 Item::getShopCost(Creature *Buyer, Creature *Seller) {
    ...
    if (isType(T_SCROLL) || isType(T_POTION)) {
        if (eID)
            cost += (TEFF(eID)->Level + 3) * (isType(T_POTION) ? 50 : 20) * 10;
...
int16 Player::FixSummonCR(int16 enCR, int16 CompType) {
    ...
    do {
        ...
        if (CompType != PHD_PARTY)
            Total += MaxGroupXCR(PHD_PARTY) - GetGroupXCR(PHD_PARTY);
