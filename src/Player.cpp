...
void Player::IntuitItem(Item *it) {
    if ((it->isKnown(KN_MAGIC) && it->isKnown(KN_PLUS)) || it->IFlags & IF_INTUITED)
        return;
...
void Character::RestEncounterChance(int16 maxDist, int16 &posseID, Creature * &in_sight, int16 &Chance) {
...
EvReturn Player::SpendHours(int16 minHours, int16 maxHours, int16 &hoursSpent, bool isHeal) {
    ...
    if ((!isHeal) && (HasStati(POISONED) || HasStati(STONING) /* || HasStati(DISEASED) */)) {
        ...
        MapIterate(m, cr, i)
            if (cr->isCreature() && cr != this)
                if (cr->DistFrom(this) <= 6)
                    if (Perceives(cr))
                        if (cr->getLeader() == this || cr->ts.hasTargetOfType(OrderTakeWatches)) {
...
int16 Game::Opt(int16 op) {
    ...
    if (isValidHandle(p[0]))
        ...
        String s = T1->IncursionDirectory + SC(T1->OptionsSubDir()) + OPT_FILE;
        if (T1->Exists(s)) {
            T1->OpenRead(s);
