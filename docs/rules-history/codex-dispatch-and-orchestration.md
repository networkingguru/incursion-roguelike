...
RUNG 2 WAS REPLACED ON 2026-09-23 (bead inc-h1bq). Brian asked why DeepSeek could not do everything Codex can, then ruled that it should take anything Codex or Sonnet would do. The limit was the client, not the model: tools/deepseek.py offers no tools and runs no loop. Measured that day: DeepSeek-V4.1-Flash on DeepInfra emitted a correct tool call and used its result. Codex CLI 0.153.4 cannot drive it, because Codex now speaks only the Responses API and DeepInfra answers HTTP 404 on /v1/openai/responses. Claude Code could drive it through DeepInfra's Anthropic endpoint, but it would load this repo's orchestrator rules, and one of them tells the agent not to author files. Brian chose opencode. In a scratch repo it read, ran a failing check, edited and re-ran it to PASS in 5 requests for about 0.0024 USD. Guards measured before adoption: with OPENCODE_DISABLE_CLAUDE_CODE unset, a canary in ~/.claude/CLAUDE.md leaked into the agent; set to 1, it did not, and the project AGENTS.md still loaded. Permission rules denied `git status` and a write outside the project. sandbox-exec with tools/opencode/sandbox.sb blocked a write to the home directory and allowed one inside. The old rung-2-versus-rung-3 test (does the work need eyes on a result?) no longer applies, because rung 2 now iterates too. Sonnet moves to a fallback for when opencode cannot run.
