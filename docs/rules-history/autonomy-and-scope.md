...
2026-09-24, bead inc-rcwt. inc-tf6l was committed and `tools/finish_bead.sh inc-tf6l` failed the merge gate on three checks, all about the check script that bead had just added: no `# gate:` tier marker (check_gate_membership.sh), no tools/README.md row (check_readme_checks.sh), and one `echo | grep -q` site (check_sigpipe_status.sh). The agent stopped, reported the three failures and asked for a go, citing save-means-stop and commit-problem-investigate-dont-dismiss. Brian's ruling, verbatim: "go. And make a rule to always fix problems in YOU SHIT if it blocks a fucking merge. Do not fuckign ask me, it's your house, keep it up." commit-problem-investigate-dont-dismiss was narrowed the same day to failures in work the agent did not produce.
