...
**Amended 2026-09-22/23 (inc-dz74):** the rule said "leave it untracked" but not in which checkout. On 2026-09-22 the landing of inc-30ps was refused because `docs/evidence/inc-30ps/` sat untracked inside the bead's worktree: `tools/finish_bead.sh` will not remove a worktree holding untracked files, since the removal would destroy them. The commit body also cited a file in that directory as the oracle for an Observed claim, so the citation would have outlived the file by minutes. The rule now splits evidence in two. The reproduction is a tool and is committed in `tools/` with the bead, always, so every claim stays re-runnable from history. The specimen is output: it lives untracked in `docs/evidence/<bead>/` in the shared checkout, which is permanent, and is committed only at the reporting-gate moment, when no worktree is left to destroy. Implementers (Codex, and DeepSeek in opencode) are sandboxed to their worktree, so they write specimens under the worktree's gitignored `logs/` and Claude copies them out before landing. The same day, `tools/__pycache__/*.pyc` blocked landings repeatedly because the gate regenerates it; that half was fixed by gitignoring `__pycache__/` and gated by `tools/check_pycache_ignored.sh`.
