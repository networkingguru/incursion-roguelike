...
- `Map: public Object` (`inc/Map.h:190`) owns `LocationInfo *Grid` (`inc/Map.h:201`), sized `sizeX*sizeY`. Serialization takes one of two paths (`inc/Map.h:668`): a v1 save embeds the grid through `V1EmbedBegin`/`GridFieldsV1` (`inc/Map.h:670-672`); the legacy v0 path writes it as one block with `r.Block` (`inc/Map.h:675`).
- `Thing: public Object` (`inc/Map.h:941`) holds `Map* m; hObj Next, hm; int16 x,y` (`inc/Map.h:967-969`). `m` is a raw pointer; `hm` is only its save form (`inc/Map.h:949-951`).
- Hierarchy: `Creature: Thing, Magic` (`inc/Creature.h:162`) -> `Character` (`inc/Creature.h:647`) -> `Player` (`inc/Creature.h:1181`); `Monster: Creature` (`inc/Creature.h:1546`). `Item: Thing, Magic` (`inc/Item.h:13`). `Feature: Thing` (`inc/Feature.h:11`) -> `Door`/`Trap`/`Portal`.
...
A Map records each Thing twice: `Map::Things`, an `NArray<hObj,1000,10>` (`inc/Map.h:244`), flat and unordered; and `LocationInfo::Contents` (`inc/Map.h:59`), the head of a per-square chain threaded through `Thing::Next` (`inc/Map.h:968`).
...
- MOUNT — `Creature::Mount` calls `Remove(false, true)`, then re-writes `x,y,m` by hand and never re-adds (`src/Skills.cpp:4328-4331`). A mount is in NEITHER list. `Thing::Remove` then skips the whole unlink block for it (`src/Display.cpp:2063`).
...
`Map::Fields` is an `OArray<Field,10,5>` of values, not pointers (`inc/Map.h:248`, next to the author's own "something is scribbling over Fields" note at `:245-247`). `LocationInfo::hasField` (`inc/Map.h:47`) caches "some field covers me"; `Map::FieldAt` short-circuits on it (`inc/Inline.h:421-431`).
...
`Thing::__Stati` is a hand-rolled `StatiCollection`, deliberately not an Array (`inc/Map.h:810-818`): live `S[]`, pending `Added[]`, per-nature index `Idx`, and a `Nested` depth so removal inside an iteration defers compaction to `_FixupStati` (`inc/Map.h:728-733`). `Thing::backRefs` (`inc/Map.h:976`) is the reverse edge; `FixupBackrefs` (`inc/Map.h:1294`) requires every `Status::h` aimed at a Thing to have a matching backref there.

## Five bugs, as invariant violations (one now fixed)

1. **wierdless in Thing::Remove** (`src/Display.cpp:2081`) violates "a Thing in `Things[]` is in the Contents chain of its own square". The walk starts at `At(ox,oy).Contents` (`:2075`) and falls off the end. On failure it returns early (`:2088`) AFTER `Things.Remove(i)` (`:2066`) and BEFORE `m = NULL` (`:2098`) — leaving the Thing in neither list with `m,x,y` intact. That early return is the zombie factory `src/MapAudit.cpp:198-204` describes. The identical walk in `Thing::Move` is `Fatal`, not `Error` (`src/Display.cpp:1786`).
2. **GetOpenXY returned (0,0)**. Fixed on 2026-09-09 (ade9c36, inc-upw.3): `Map::GetOpenXY` (`src/MakeLev.cpp:3596-3620`) now returns `NO_OPEN_XY` when `OpenC` is zero, and its caller `Thing::PlaceOpen` (`inc/Map.h:1044-1059`) disposes the Thing with `Remove(true)` instead of placing it in the solid corner. The invariant it broke, "the open-square set belongs to the map being populated", still does not hold: `OpenX/OpenY/OpenC` are `static` members of Map (`inc/Map.h:215-216`), shared by every Map instance and by encounter generation (`src/Encounter.cpp:2607`, `:2629`). Every script call site guards with `FindOpenAreas` (`lib/program.i`), so reaching the empty case needs `OpenC` clobbered between guard and use.
...
4. **Encounter placed, no creature** (`src/MakeLev.cpp:3507`, `ASSERT(c = GetEncounterCreature(0))`) violates "`CandidateCreatures[i]` refers to a live, placed creature". `enBuildMon` writes the slot at `src/Encounter.cpp:2591`, then 33 lines later can delete the monster after 50 failed terrain tries (`:2624`, `mn->Remove(true); return ABORT;`) without clearing it. The array is memset only on a non-nested generate (`src/Encounter.cpp:536`), and `GetEncounterCreature` returns the raw pointer with no liveness check (`src/Encounter.cpp:365-371`). `ASSERT` is always live — it calls `Error` (`inc/Defines.h:101`).
5. **orphans** (`src/MapAudit.cpp:224`). The two lists are `Map::Things` and the per-square Contents chains. Check 3 tests only `inThingsArray` (`:222`), and it exempts MOUNT/ENGULFED (`:220`) exactly as check 1 does (`:156`). Mounts are in neither list by design (`src/Skills.cpp:4328-4331`). That exemption landed on 2026-08-17; every orphan line in the log comes from a session before it, names a mountable creature, and reports `F_DELETE=0`.
