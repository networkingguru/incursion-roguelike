...
- Hierarchy: `Creature: Thing, Magic` (`inc/Creature.h:160`) -> `Character` (`inc/Creature.h:638`) -> `Player` (`inc/Creature.h:1172`); `Monster: Creature` (`inc/Creature.h:1533`). `Item: Thing, Magic` (`inc/Item.h:13`). `Feature: Thing` (`inc/Feature.h:11`) -> `Door`/`Trap`/`Portal`.
...
A Map records each Thing twice: `Map::Things`, an `NArray<hObj,1000,10>` (`inc/Map.h:231`), flat and unordered; and `LocationInfo::Contents` (`inc/Map.h:56`), the head of a per-square chain threaded through `Thing::Next` (`inc/Map.h:930`).
...
- MOUNT — `Creature::Mount` calls `Remove(false)`, then re-writes `x,y,m` by hand and never re-adds (`src/Skills.cpp:4301-4304`). A mount is in NEITHER list. `Thing::Remove` then skips the whole unlink block for it (`src/Display.cpp:2026`).
...
FI_SIZE (`inc/Defines.h:3457`) is the bulk of a creature above size Large, created at `src/Monster.cpp:1604` and `src/Values.cpp:1702`; `GetAt` treats it as a creature standing on every covered square (`src/Display.cpp:1527-1539`).

`Thing::__Stati` is a hand-rolled `StatiCollection`, deliberately not an Array (`inc/Map.h:772-780`): live `S[]`, pending `Added[]`, per-nature index `Idx`, and a `Nested` depth so removal inside an iteration defers compaction to `_FixupStati` (`inc/Map.h:690-695`). `Thing::backRefs` (`inc/Map.h:938`) is the reverse edge; `FixupBackrefs` (`inc/Map.h:1230`) requires every `Status::h` aimed at a Thing to have a matching backref there.

## The five open bugs, as invariant violations

1. **wierdless in Thing::Remove** (`src/Display.cpp:2044`) violates "a Thing in `Things[]` is in the Contents chain of its own square". The walk starts at `At(ox,oy).Contents` (`:2038`) and falls off the end. On failure it returns early (`:2051`) AFTER `Things.Remove(i)` (`:2029`) and BEFORE `m = NULL` (`:2061`) — leaving the Thing in neither list with `m,x,y` intact. That early return is the zombie factory `src/MapAudit.cpp:198-204` describes. The identical walk in `Thing::Move` is `Fatal`, not `Error` (`src/Display.cpp:1749`).
2. **GetOpenXY returns (0,0)** (`src/MakeLev.cpp:3572-3579`) violates "the open-square set belongs to the map being populated". `OpenX/OpenY/OpenC` are `static` members of Map (`inc/Map.h:202-203`), shared by every Map instance and by encounter generation (`src/Encounter.cpp:2547`, `:2569`). The caller is `Thing::PlaceOpen` (`inc/Map.h:995-997`), which decodes 0 as (0,0) unchecked. Every script call site does guard with `FindOpenAreas` (`lib/program.i`), so the failure needs `OpenC` clobbered between guard and use.
...
- `src/Display.cpp:2156` `Item::Remove` zeroes `Next` before calling `Thing::Remove` at `:2161`, which then writes that zero into the predecessor's `Next` (`:2056`), truncating the square's chain. Reachable only if an Item has both `Parent` and `m` set. Read, not observed.
...
- `src/Display.cpp:1685` `Thing::Move` caches `Map *M = m`, then throws field events (`:1703-1728`) that can re-place this Thing on another map; the unlink and insert afterwards use the stale `M`.
