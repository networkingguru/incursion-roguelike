...
Most other C++ call sites pass `safe=true` — stairs ([Feature.cpp:260](https://github.com/rmtew/incursion-roguelike/blob/master/src/Feature.cpp#L260)), `Skills.cpp:4008`, `:4066`, `:4161`, `:4181`, `Prayer.cpp:1482` — so if the arrival square is a fall square the code steps off it; they enter the cycle only when the landing square and all eight of its neighbours are fall squares. Two of them (`Skills.cpp:4008`, `:4066`) ascend rather than descend and cannot reach the `BELOW_DUNGEON` branch at all. One does not: wizard-mode Ascend/Descend at `Debug.cpp:810` calls `MoveDepth(i)` with no second argument. That is the frame directly beneath the outer `MoveDepth` in the crash report above, and I trimmed it out of the excerpt, which I should not have done.
