...
`safe` decides only whether the *cycle* is entered, not whether the fault is reached. The null read is at the top of the function; the `safe` test is forty lines below it, so a call from the bottom level faults before `safe` is ever looked at. That matters for one of the call sites in particular: `Skills.cpp:4181` is the `>` command's *"Climb down the chasm?"* branch, and on the bottom level it calls `MoveDepth(m->Depth + 1, true)` and dereferences `RES(0)` exactly as the wizard path does. No wizard mode, no script, no trap, no recursion — a character who climbs down a chasm on depth 10 hits it.

What `safe=true` does buy is that the arrival point steps off a fall square, so those sites enter the *cycle* only when the landing square and all eight of its neighbours are fall squares. Three of the six cannot reach the `BELOW_DUNGEON` branch at all in any case: `Skills.cpp:4008` and `:4066` ascend, and `Prayer.cpp:1482` passes a literal depth of 1. One does not: wizard-mode Ascend/Descend at `Debug.cpp:810` calls `MoveDepth(i)` with no second argument. That call site sits in `Player::WizardOptions`, the frame at the bottom of the crash report above: my harness reached depth 10 by that route, so the outer call in the trace is itself an unsafe one.
...
The patch's own cost is an A/B on one keyword. Two builds of my tree, same command and same flags, both carrying my current diagnostic probes — a fatter set than the 16-byte one above, which is why both frames are large — so that they cancel. The only difference is the `static` on `Thing *GoWith[64]`:
...
One thing worth saying plainly, since the rest of this answer is about cost: those 512 bytes buy correctness rather than tidiness. `static` means one array for the whole process, so in the recursion (a) describes the nested call refills `GoWith` while the outer call still holds its own `gwc` and has not yet reached `GoWith[i]->PlaceAt(new_m, nx, ny)` at [Feature.cpp:937-938](https://github.com/rmtew/incursion-roguelike/blob/master/src/Feature.cpp#L937-L938). The outer call then places whatever the inner one left there. That is what the patch is for; the stack is what it costs.
