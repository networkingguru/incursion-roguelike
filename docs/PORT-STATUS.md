...
**Narrowing the typedefs then broke saving.** `Thing`'s archive did
...
Fixed by dropping the cast; the reasoning is recorded at `inc/Map.h:920`, which
...
| 58 format-string defects | 26 residual type mismatches, plus 32 that are live bugs on Windows too. **2026-08-17: the enumeration step written here is already done and is now a no-op.** `Format()` carries `__attribute__((format(printf,1,2)))` at `inc/Base.h:185`, as does `Error()`. So the 58-defect count cannot be reproduced by "add the attribute"; it needs a compile with the attributes already in place. Note that `build_macos.sh:303` puts `-w` in `CFLAGS` outright, and `:302` puts `$WARN_FLAGS` in `CXXFLAGS`, which defaults to `-w` at `:157`; `-w` silences every warning, so a normal build shows none of these. Set `WARN_FLAGS` to something else to see them -- the switch exists for exactly this, and `:145-151` explains why appending `-Wformat` to `-w` does not work. |
