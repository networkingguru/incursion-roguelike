...
`src/Move.cpp:1541`, inside `Creature::TerrainEffects` at `src/Move.cpp:1402`). The re-entry path is ordinary event dispatch; no
...
the top byte, `Modules[(xID >> 24)-1]`, `MAX_MODULES` being 126 (`inc/Defines.h:4452`), so an id with a zero top byte indexed
...
shape at `src/Creature.cpp:818`, `src/Magic.cpp:1261`.
