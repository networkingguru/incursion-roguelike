...
| `INCURSION_DUNGEONMAP_PROBE`, `INCURSION_DESCEND_PROBE` | Dungeon and descent structure. | **BOTH GONE.** `INCURSION_DESCEND_PROBE` went in `ccf91de`. `INCURSION_DUNGEONMAP_PROBE` went with the fix it found, inc-tos: its reading survives as the log it wrote, `docs/evidence/inc-x9i/observed-routes/dungeonmapprobe-levitate-seed777.log` and its seed 111 twin, and the state it reported is now driven as ordinary gameplay by `tools/check_dungeonmap_bounds.sh`. |
