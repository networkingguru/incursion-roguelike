...
9. `Creature::Walk()` `src/Move.cpp:76` is the handler `EV_MOVE` reaches (`:147`); it ends at `Move(tx, ty, ...)` (`src/Move.cpp:1045`).
...
- `inc/Term.h:678` — `RefreshMap()` dereferences `p` unchecked and is called as `T1->RefreshMap()` (`src/Magic.cpp:1402`, `src/Skills.cpp:1943`); `ClearPlayer()` (`inc/Term.h:774`) sets `p` to NULL, so the state is reachable.
