...
(`extern Term *T1`, `inc/Globals.h:289`), and a ruleset that is data, not code:
...
- `inc/Term.h:678` — `RefreshMap()` dereferences `p` unchecked and is called as `T1->RefreshMap()` (`src/Magic.cpp:1421`, `src/Skills.cpp:1943`); `ClearPlayer()` (`inc/Term.h:774`) sets `p` to NULL, so the state is reachable.
