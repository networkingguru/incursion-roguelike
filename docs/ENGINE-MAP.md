...
All three `main()`s do the same five things in order: `new Game`, `new <backend>`, `SetIncursionDirectory`, `T1 = AT1`, then `RunOnCommandLine` or `Initialize`/`StartMenu`/`ShutDown` (`src/Wposix.cpp:564-613`, `src/Wlibtcod.cpp:674-709`, `src/Wcurses.cpp:482-492`). Each defines its own `Term *T1` (`src/Wposix.cpp:282`, `src/Wlibtcod.cpp:544`, `src/Wcurses.cpp:406`), so exactly one backend can link.
