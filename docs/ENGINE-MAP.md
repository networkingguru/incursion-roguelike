...
6. `switch (ch)` `src/Player.cpp:380`; a direction key lands at `src/Player.cpp:1268` and, with no attack chosen, throws `ThrowDir(EV_MOVE, ...)` (`src/Player.cpp:1367`).
...
9. `Creature::Walk()` `src/Move.cpp:24` is the handler `EV_MOVE` reaches (`:147`); it ends at `Move(tx, ty, ...)` (`src/Move.cpp:993`).
...
- `inc/Term.h:678` — `RefreshMap()` dereferences `p` unchecked and is called as `T1->RefreshMap()` (`src/Magic.cpp:1364`, `src/Skills.cpp:1943`); `ClearPlayer()` (`inc/Term.h:774`) sets `p` to NULL, so the state is reachable.
