...
2. `Game::StartMenu()` `src/Main.cpp:2162`; menu choice 0 runs `LoadModules()` (`src/Registry.cpp:1473`), `NewGame()` (`src/Main.cpp:90`), `Play()` (`:2233-2245`).
...
10. `Thing::Move()` `src/Display.cpp:1699` relinks square contents, calls `M->Update()` on both changed squares (`:1798-1799`), and sets the player's `UpdateMap` (`:1814`).
...
**Invariant.** `Player::UpdateMap` is a one-bit dirty flag. Only `ShowMap` clears it (`src/Term.cpp:945`); every world change sets it (`src/Display.cpp:1814,1618`). A map that will not redraw is nearly always a lost set of that flag, not a broken draw call.
