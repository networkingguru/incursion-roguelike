...
| `check_prime_budget.sh` | Does the SessionStart memory injection still fit in one piece? A hook payload the host judges too large is replaced by a 2 KB preview and written to a file nobody reads, so the project's rules never reach the session; the failure is silent and sits on the delivery side, not in the content. It asserts the hook JSON contract, that `tools/prime_rules.py` stays under the budget declared at the top of that script, that the block names how many memories it withheld, and that the specific rules already broken for want of context are still inside it. |
