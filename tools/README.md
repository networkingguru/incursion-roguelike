...
| `check_blind_fight_miss.sh` | Does an attacker WITH the Blind-Fight feat, attacking a victim it cannot see, get the 25% miss chance the game's own help promises -- rather than the 50% the buggy `HasStati(FT_BLIND_FIGHT)` gives when the attacker is not CHARGING (FT_BLIND_FIGHT == CHARGING == 75)? | LIVE |
...
| `check_feed_upon_pain.sh` | Does Feed upon Pain heal from a running tally of hits of 10 damage or less, and does a level-6 frost giant heal 6 HP per 10 damage (bead inc-akac)? Needs the headless build. | LIVE |
