...
if [ "$STAMP" != "IS1.2" ]; then
    fail "save Version field reads \"$STAMP\", wanted \"IS1.2\" -- real saves are not at the current v1 revision"
fi
...
grep -qE '^Format:    IS1\.2$' "$WORK/dump1.txt" ||
    fail "Format: line missing or not IS1.2 -- -dump does not name the file's own stamp"
